...
import useAudioRecorder from "C:\TranslateProject\src\hooks\useAudioRecorder.js"
...
export const TranslationProvider = ({ children }) => {
  ...
  // --- [State] 변하는 데이터 관리 ---
  const [messages, setMessages] = useState([]); // 채팅 기록 (화면에 보여질 리스트)
  const [sourceLang, setSourceLang] = useState('ko'); // 입력 언어 (기본: 한국어)
  const [targetLang, setTargetLang] = useState('en'); // 번역할 언어 (기본: 영어)
  const [isLoading, setIsLoading] = useState(false);  // 로딩 중인지 여부 (스피너 표시용)
  ...
  const handleSendMessage = async (text) => {
    ...
    try {
      ...
      console.log(`[API 요청] 입력: ${text}, 소스: ${sourceLang}, 타겟: ${targetLang}`);
      
      // -- 가짜 API 시뮬레이션 시작 --
      await new Promise(resolve => setTimeout(resolve, 1000)); 
      const dummyTranslatedText = `[번역됨] ${text}`; 
      // -- 가짜 API 시뮬레이션 끝 --

      // 2. 번역된 결과(또는 응답)를 화면에 표시
      addToMessageList(dummyTranslatedText, false);
